...
from yolox.tracker.gbyte_tracker import GBYTETracker
...
def make_parser():
    ...
    parser.add_argument("--model_folder", type=str, default='pretrained/ckpt_car586.t791', help="reid model folder")
    return parser
    ...
    while True:
        ...
        if ret_val:
            ...
            if outputs[0] is not None:
                ...
                timer.toc()
                online_im = plot_tracking(img_info['raw_img'], online_tlwhs, online_ids, frame_id=frame_id + 1)
                                          # fps=1. / timer.average_time)   #删除了fps的显示 在这个函数里面改
            else:
                timer.toc()
                online_im = img_info['raw_img']
            if args.save_result:
