...
DATA_PATH = '../datasets/UAV'
OUT_PATH = os.path.join(DATA_PATH, 'annotations')
SPLITS = ['test']  # --> split training data to train_half and val_half.
HALF_VIDEO = True                                 # 'train_half', 'val_half', 'train', 'test'
...
if __name__ == '__main__':
    ...
    for split in SPLITS:
        ...
        for seq in sorted(seqs):
            ...
            ann_path = os.path.join(seq_path, 'gt/gt_whole.txt')     # gt.txt
            ...
            for i in range(num_images):
                if i < image_range[0] or i > image_range[1]:
                    continue
                img = cv2.imread(os.path.join(data_path, '{}/img1/img{:06d}.jpg'.format(seq, i + 1)))    # 之前没有img
            ...
            if split != 'test1':                                                         # test
                ...
                for i in range(anns.shape[0]):
                    ...
                    cat_id = int(anns[i][8])    # int(anns[i][7])
                    ann_cnt += 1
                    ...
                    if not ('15' in DATA_PATH):
                        # if not (float(anns[i][8]) >= 0.25):  # visibility.
                        # continue
                        if int(anns[i][6]) == 5:  # ignore medium out
                            continue
                        # if int(anns[i][7]) in [3, 4, 5, 6, 9, 10, 11]:  # Non-person
                        #     continue
                        # if int(anns[i][7]) in [2, 7, 8, 12]:  # Ignored person
