...
# Use the same script for MOT16         'train_half', 'val_half', 'train', 'test'
DATA_PATH = '../datasets/visdrone'
...
if __name__ == '__main__':
    ...
    for split in SPLITS:
        ...
        out = {'images': [], 'annotations': [], 'videos': [],
               'categories': [{'id': 1, 'name': 'pedestrian'}, {'id': 4, 'name': 'car'}, {'id': 6, 'name': 'truck'},
                              {'id': 9, 'name': 'bus'}]}
        # out = {'images': [], 'annotations': [], 'videos': [],
        #        'categories': [{'id': 1, 'name': 'pedestrian'}, {'id': 2, 'name': 'pedestrian'}, {'id': 3, 'name': 'pedestrian'}]}
        # 'categories': [{'id': 1, 'name': 'pedestrian'}, {'id': 2, 'name': 'people'},
        #                               {'id': 3, 'name': 'bicycle'}, {'id': 4, 'name': 'car'}, {'id': 5, 'name': 'van'},
        #                               {'id': 6, 'name': 'truck'}, {'id': 7, 'name': 'tricycle'},
        #                               {'id': 8, 'name': 'awning-tricycle'}, {'id': 9, 'name': 'bus'},
        #                               {'id': 10, 'name': 'motor'}]}
        ...
        for seq in sorted(seqs):
            ...
            for i in range(num_images):
                if i < image_range[0] or i > image_range[1]:
                    continue
                img = cv2.imread(os.path.join(data_path, '{}/img1/{:07d}.jpg'.format(seq, i + 1)))  # 06d
            ...
            if split != 'test1':
                ...
                if CREATE_SPLITTED_ANN and ('half' in split):
                    ...
                    for o in anns_out:
                        fout.write('{:d},{:d},{:d},{:d},{:d},{:d},{:d},{:d},{:d},{:d}\n'.format(
                                    ...
                                    int(o[6]), int(o[7]), int(o[8]), int(o[9])))     # {:d},{:d},{:d},{:d},{:d},{:d},{:d},{:d},{:.6f}
                    fout.close()
                ...
                for i in range(anns.shape[0]):
                    ...
                    cat_id = int(anns[i][7])    # int(anns[i][7])
                    ann_cnt += 1
                    if not ('15' in DATA_PATH):
                        ...
                        if not (int(anns[i][6]) == 1):  # whether ignore.
                            continue
                        if int(anns[i][7]) in [0, 2, 3, 7, 8, 10, 11]:  # Non-person
                            continue
                        if int(anns[i][7]) in [5]:  # Ignored person
                            category_id = 6
                            continue
                        else:
                            category_id = cat_id  # pedestrian(non-static)
